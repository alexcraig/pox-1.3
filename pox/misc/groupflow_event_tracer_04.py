...
class GroupFlowTraceEvent(TraceEvent):
    ...
    * bloom_filter_calc_start_time: Time at which bloom filter construction was started for this routing event (if applicable)
    * bloom_filter_calc_end_time: Time at which bloom filter construction was completed for this routing event (if applicable)
    ...
    * src_ip: Multicast sender IP address which this routing event is associated with.
    ...
    def set_bloom_filter_calc_start_time(self):
        """Records the current time as the time at which bloom filter calculation was initiated."""
        self.bloom_filter_calc_start_time = self.get_curr_time()
    ...
    def get_bloom_filter_calc_time(self):
        """Returns the length of time (in seconds) associated with bloom filter calculation for this routing event.

        Returns None if the associated times have not been recorded.
        """
        if not self._complete_bloom_filter_calc:
            return None

        return self.bloom_filter_calc_end_time - self.bloom_filter_calc_start_time
