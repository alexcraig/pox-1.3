...
class FlowTrackedSwitch(EventMixin):
    ...
    def __init__(self, flow_tracker):
        ...
        self.num_cookied_flows = 0 # Tracks all flows that has been assigned a cookie, regardless of whether they output to an active port
    ...
    def set_tracked_ports(self, tracked_ports):
        ...
        * tracked_ports: List of integer port numbers on which utilization should be tracked for this switch
        ...
        Note: The special port value assigned to bloom filter forwarding is always included
        """
    ...
    def process_port_stats(self, stats, reception_time):
        ...
        if not self.flow_tracker._log_file is None:
            ...
            self.flow_tracker._log_file.write('PortStats Switch:' + dpid_to_str(self.dpid) + ' NumFlows:' + str(
                self.num_cookied_flows) + ' IntervalLen:' + str(
	...
	# log.debug(str(stats))
        ...
        for port_no in self.tracked_ports:
            ...
            if not port_no in self.flow_total_byte_count:
                self.flow_total_byte_count[port_no] = {}
                self.flow_interval_byte_count[port_no] = {}
                self.flow_interval_bandwidth_Mbps[port_no] = {}
                self.flow_average_bandwidth_Mbps[port_no] = {}
        ...
        self.num_cookied_flows = num_cookied_flows
        if not self.flow_tracker._log_file is None:
            self.flow_tracker._log_file.write('FlowStats Switch:' + dpid_to_str(self.dpid) + ' NumFlows:' + str(
                self.num_cookied_flows) + ' IntervalLen:' + str(interval_len) + ' IntervalEndTime:' + str(
